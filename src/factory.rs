...
impl Factory {
    pub fn new(address: Option<Address>, chain: Chain, protocol: Protocol) -> Self {
        Self {
            address: address.unwrap_or_else(|| protocol.get_addresses(chain).1),
            chain,
            protocol,
        }
    }
    ...
    pub fn codehash(&self) -> H256 {
        match self.protocol {
            ...
            Protocol::Custom(router, factory) => {}
        }
    }
}
